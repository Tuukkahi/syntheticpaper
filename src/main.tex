...
Here, we present a method to compute optimal state estimates for advective time series data.
To use indirect, partial, or multiple observation sources with the neural network, and for uncertainty quantification, we employ the Kalman filter.
Because of large dimensionality of the image data, the standard Kalman filter is computationally prohibitive, and hence we compute the covariance estimates in a reduced dimension.

A general linear Gaussian state space model can be given as
\begin{align}
    x_{k} &= M_{k} x_{k-1} + W_{k+1}, \qquad W_{k+1} \sim \mathcal{N}(0, Q_{k+1})\\
    y_{k} &= H_{k}x_{k} + V_{k}, \qquad V_{k} \sim \mathcal{N}(0, R_{k}).
\end{align}
Here $M_{k} \in \mathbb{R}^{d \times d}$ is the state model that maps the state in time, and $H_{k} \in \mathbb{R}^{m \times d}$ the observation model that takes states into observations.
The observation and state error processes $V_k$, and $W_k$ are assumed additive Gaussians with zero mean.

In our application the state model $M_{k}$ is the semi-Lagrangian operator for which the neural network approximates the vector field from previous states.
The observation operator $H_{k}$ is to be tailored for the type of observations.
In our work the observations are direct but partial, and hence is the identity operator for the observed pixels.

In Kalman filtering, one performs two steps: prediction step and update step.
Prediction step consists of propagating the state vector and covariance to obtain the conditional density
\begin{equation}
    \pi(x_{k+1} \, \vert \, y_{1:k}) \sim \mathcal{N}(x_{k+1 \, \vert \, k}, C_{k+1 \vert k}),
\end{equation}
by computing the following:
\begin{align}
    x_{k \, \vert \, k-1} &= M_{k} x_{k-1 \, \vert \, k-1}\\
    C_{k \, \vert k-1} &= M_{k} C_{k-1 \, \vert \, k-1} M_{k}^{\top} + Q_{k}.
\end{align}
Update step updates the predicted mean and covariance with a new observation to obtain the posterior density
\begin{equation}
    \pi(x_{k} \, \vert \, y_{1:k}) \propto \exp\left( -\frac{1}{2} ( \Vert x_{k \, \vert \, k-1} - x_{k} \Vert_{C_{k \, \vert k-1}}^{2} + \Vert y_{k} - H_{k}x_{k} \Vert_{R_{k}}^{2}) \right),
\end{equation}
which can be computed using the standard Kalman filter formulas, see e.g.~\cite{kaipiosomersalo}.
Because these formulas involve inverting the innovation matrix, which is of same shape as the observation's covariance and thus it becomes unfeasible even for small image data.

Hence we use the prior-based dimension reduction technique for devising the Kalman filter formulas following the original paper~\cite{solonen} that extends the standard prior-based dimension reduction for static linear Gaussian inverse problems for the Kalman filter setting.
First we parameterize the state with a fixed projection matrix $P_r \in \mathbb{R}^{d \times r}$ as
\begin{equation}
    x_{k} = x_{k \, \vert \, k-1} + P_r \alpha_k.
\end{equation}
With this, the prediction step reads 
\begin{align}
    x_{k \, \vert \, k-1} &= M_{k} (x_{k-1} + P_r \alpha_{k-1})\\
    C_{k \, \vert k-1} &= (M_{k} P_r) \Psi_{k-1} (M_{k} P_r)^{\top} + Q_{k}, \label{eq:C}
\end{align}
where $\Psi$ is the covariance of the subspace coordinates $\alpha$.
This prior then yields the posterior density
\begin{equation}
    \pi(\alpha_{k} \, \vert \, y_{1:k}) \propto \exp\left( -\frac{1}{2} ( \Vert P_r \alpha_k \Vert_{C_{k \, \vert k-1}}^{2} + \Vert y_{k} - H_{k}x_{k} H_{k}P_{r}\alpha_{k} \Vert_{R_{k}}^{2} ) \right).
\end{equation}
Then the posterior mean and covariance for $\alpha_k$ are given by
\begin{align}
    \alpha_{k} &= \Psi_{k} (H_k P_r)^{\top} R_{k}^{-1} (y_k - H_k x_{k \, \vert \, k-1})\\
    \Psi_{k} &= \left( (H_k P_r)^{\top} R_{k}^{-1} (H_k P_r) + P^{\top} C_{k \, \vert \, k-1}^{-1} P_r\right)^{-1} \label{eq:psi}
\end{align}
To evaluate the high-dimensional inverse $C_{k \, \vert \, k-1}^{-1}$ in~\ref{eq:psi}, we introduce a short-hand notation $B_k = M_k P_r A_k \in \mathbb{R}^{d \times r}$ with $A_k$ the matrix square root $\Psi_{k-1} = A_{k} A_{k}^{\top}$, and use~\ref{eq:C} to write
\begin{equation}
    C_{k \, \vert \, k-1}^{-1} = \left( (M_{k} P_r) \Psi_{k-1} (M_{k} P_r)^{\top} + Q_{k} \right)^{-1} = (B_k B_{k}^{\top} + Q_k)^{-1},
\end{equation}
where $B_k = M_k P_r A_k \in \mathbb{R}^{d \times r}$ with $A_k$ the matrix square root $\Psi_{k-1} = A_{k} A_{k}^{\top}$.
The Sherman-Morrison-Woodbury inversion formula allows us to turn this into 
\begin{equation}
    C_{k \, \vert \, k-1}^{-1} = Q_{k}^{-1} - Q_{k}^{-1} B_k(I + B_{k}^{\top} Q_{k}^{-1} B_{k})^{-1}B_{k}Q_{k}^{-1}.
\end{equation}
In filtering, we typically assume that the model error covariance $Q_{k}$ is diagonal, and hence we now need to invert only a $r \times r$ matrix to compute the filter.
